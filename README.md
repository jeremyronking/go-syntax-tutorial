...
**Four takes on the same Go tutorial.**
...
- **go-minimax** — Built by MiniMax (via Ollama cloud) in OpenAI's Codex CLI. The MiniMax take on the same lesson set. → [go-minimax.jking.ai](https://go-minimax.jking.ai)
...
The [spec](./site/spec.md) is the brief that defines the product. The [prompt](./site/prompt.md) is the meta-instruction that told each agent how to work. The [issues](./site/issues/epic.md) are the phased breakdown all four followed.

This repo holds the read-only docs surface (under `site/`) and the four app implementations (`go-claude/`, `go-gemini/`, `go-glm/`, `go-codex-minimax/`). The four apps themselves are linked above.
